...
export const StyledPost = ({ post, user, allTags, onNotifAction }) => {
  ...
  const handleLikes = async (e) => {
    ...
    let targetUser = await axios.get(
      `http://localhost:5000/api/users/?username=${post?.username}`
    );
  ...
  useEffect(() => {
    const fetchUserInfo = async () => {
      const userInfo = await axios.get(
        `/users/?username=${currentPost.username}`
      );
          ...
          <div class="card__footer">
            <div class="user">
              <div class="user__info">
                {author?.profilePic ? (
                  <img src={PF + author?.profilePic} class="user__image" />
                ) : null}
                <div>
